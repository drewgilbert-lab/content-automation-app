...
**I6 — Skill Testing Interface** — **Done**
Built `/skills/[id]/test` page where a user can run a skill against sample inputs and preview the generated output. Accepts a content type, optional persona/segment/use case context, and a test prompt. Calls `assembleContext()` with manual skill selection and streams Claude's response. Includes side-by-side comparison of output with and without the skill applied. System prompt preview (collapsible) shows the full assembled prompt. API routes: `POST /api/skills/[id]/test` (streaming generation), `GET /api/skills/[id]/test/context` (context option dropdowns). "Test" button added to skill detail page header.

**I7 — Claude Skill Package Compatibility Standard** — **Done**
Defined and implemented a canonical Claude-compatible skill package format with bidirectional conversion between internal `Skill` objects and `SKILL.md` bundles. Core library (`lib/skill-package.ts`): `skillToPackage()`, `packageToSkillInput()`, `generateSkillMd()`, `parseSkillMd()`, `validateSkillPackage()`, `generateMetadataJson()`. Types in `lib/skill-package-types.ts`. API routes: `GET /api/skills/[id]/export` (downloads `.skill` ZIP with `SKILL.md` + `metadata.json`), `POST /api/skills/import` (parses uploaded SKILL.md or .skill ZIP, returns `SkillCreateInput` for review). CLI packager: `scripts/package-skill.ts` (reimplements Claude's `package_skill.py` in TypeScript). UI: "Export" button on skill detail page, "Import" button on skills list page with sessionStorage-based form prefill. Template at `.claude/skills/_template/SKILL.md`. Standards documented in `docs/CLAUDE_SKILLS_STANDARD.md`.

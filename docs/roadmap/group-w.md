...
| Create/edit knowledge objects and skills | Yes | Yes (direct) | Yes (routed through review queue) | No |
...
| Access the Generate UI | Yes | Yes | Yes | No |
| View cost dashboard | Yes | Yes | Yes | Yes |
| Configure system settings | Yes | No | No | No |

The Admin role has full platform access including user management and system configuration. The Editor role is focused on content governance — Editors can review, approve, and reject submissions, use AI Merge, and create or edit content directly without going through the review queue. Contributors propose changes via the review queue but cannot approve them. Viewers have read-only access.

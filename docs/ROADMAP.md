...
**M1 — `sourceKnowledgeObjects` Field on Skill Collection**
...
**M3 — `buildSkillRefreshPrompt` in `lib/merge.ts`**
...
**M7 — Bootstrapping: Suggested Links via Semantic Similarity**
...
When an external system pushes a knowledge object update via MCP (Group J) and that submission is accepted, the same Group M skill refresh pipeline fires automatically. Real-world intelligence from CRM syncs, competitive intel feeds, or Slack bots could cascade into skill refresh suggestions — the admin reviews in the queue as normal.
...
> Dependencies: Groups A–B (knowledge CRUD), Group I (Skills Module), Group K (REST API semantic search — replaced/enhanced), Group J (MCP Server — enhanced in future phase).
...
Same pattern for `/skills/[id]`. Enables questions like "Which knowledge objects are most relevant to the Campaign Brief Generator skill?" — useful for the Group M knowledge-linked skills bootstrapping (M7 Suggest Links).
...
No code changes are needed in the Ask route, Ask UI, MCP tools, or REST API endpoints — all downstream consumers read from the registry. This process should be added to the schema change checklist in `.cursor/rules/schema-change.mdc` (Group N2).
...
> Dependencies: Groups A–F (knowledge CRUD, submission/review queue, AI merge), Group I (Skills Module — context assembly, Content Narrative Generator skill), Group K (External REST API — extended with narrative endpoints), Group J (MCP Server — extended with narrative tools).
...
| External REST API (Group K) | New endpoints: `GET /api/v1/narratives` (list approved), `GET /api/v1/narratives/:id` (detail). Read-only. |
| MCP Server (Group J) | New tools: `list_narratives`, `get_narrative`, `create_narrative_draft`. Allows AI agents and automation to discover and use narratives. |
...
| Weaviate Query Agent | Group Q — not yet started. Query Agent retrieval + Claude synthesis, Ask UI, collection registry | See Group Q above |
| Content Narratives | Group R — not yet started. Strategic narrative layer between knowledge and content generation, AI-assisted creation, review workflow, context assembly integration, staleness detection | See Group R above |
